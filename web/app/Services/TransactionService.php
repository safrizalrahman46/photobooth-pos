...
class TransactionService
{
    ...
    public function addExtraPrintBulk(Transaction $transaction, array $payload, int $cashierId): Transaction
    {
        return DB::transaction(function () use ($transaction, $payload, $cashierId): Transaction {
            /** @var Transaction $lockedTransaction */
            $lockedTransaction = Transaction::query()
            ...
            foreach ($items as $item) {
                ...
                if ($addOn->package_id !== null && ! $packageIds->contains((int) $addOn->package_id)) {
                    throw ValidationException::withMessages([
                        "items.{$item['add_on_id']}.add_on_id" => 'Add-on tidak sesuai dengan paket transaksi ini.',
                    ]);
                }
                ...
                if ($qty > $maxQty) {
                    throw ValidationException::withMessages([
                        "items.{$item['add_on_id']}.qty" => sprintf('Maksimum qty untuk %s adalah %d.', (string) $addOn->name, $maxQty),
                    ]);
                }
            ...
            if ($paymentAmount < $totalLineTotal) {
                throw ValidationException::withMessages([
                    'paid_amount' => 'Nominal pembayaran tidak boleh kurang dari total tambah cetak.',
                ]);
            }

            $subtotal = (float) $lockedTransaction->subtotal + $totalLineTotal;
            $discount = (float) $lockedTransaction->discount_amount;
            $tax = (float) $lockedTransaction->tax_amount;
            $total = max(0, $subtotal - $discount + $tax);
            $paid = (float) $lockedTransaction->paid_amount;
            ...
            $updatedTransaction = $this->addPayment($lockedTransaction, [
                ...
                'amount' => $paymentAmount,
                'reference_no' => $payload['reference_no'] ?? null,
                'notes' => $payload['notes'] ?? 'Tambah cetak (gabungan).',
                'meta' => $meta,
            ], $cashierId);
            ...
            $this->activityLogger->log(
                'transactions',
                'extra_print_bulk_added',
                $cashierId,
                Transaction::class,
                (int) $lockedTransaction->id,
                [
                    'message' => sprintf('Tambah cetak gabungan (%d item) ditambahkan ke transaksi %s.', count($createdLines), (string) $lockedTransaction->transaction_code),
                    'label' => (string) $lockedTransaction->transaction_code,
                    'item_count' => count($createdLines),
                    'items' => $addOnDetails,
                    'total_line_total' => $totalLineTotal,
                    'payment_amount' => $paymentAmount,
                ],
            );
